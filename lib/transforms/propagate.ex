defmodule Paradigm.Transform.Propagate do
  ...
  defp do_conformance_check(universe, instantiation_node_id) do
    paradigm_graph_node =
      Paradigm.Graph.follow_reference(universe, instantiation_node_id, "paradigm")

    instance_graph_node =
      Paradigm.Graph.follow_reference(universe, instantiation_node_id, "instance")
  ...
  defp apply_transform_if_needed(universe, transform_node_id) do
    ...
    matching_instance_ids =
      Enum.filter(instantiation_nodes, fn instantiation_node ->
        ...
        paradigm_id == source_paradigm_id and
  ...
  defp create_transform_instance(universe, transform_node_id, source_graph_id) do
    ...
    {:ok, result_graph} =
      apply(transform_node.data["module"], :transform, [
        source_graph,
        Paradigm.Graph.MapGraph.new(source_graph.metadata),
        %{}
      ])
    ...
    transform_instance_id =
      "#{transform_node.data["name"]}_from_#{source_graph_id}_to_#{target_id}"
    ...
    |> Paradigm.Universe.insert_graph_with_paradigm(
      result_graph,
      registered_graph.data["name"],
      transform_node.data["target"].id
    )
  end
end
